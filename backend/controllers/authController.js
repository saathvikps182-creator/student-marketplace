...
const register = async (req, res) => {
  const { name, email, usn, password, college_name } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (name, email, usn, password, college_name)
       VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, usn, college_name`,
      [name, email, usn.toUpperCase(), hashedPassword, college_name || '']
    );
...
const login = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      token,
      user: { id: user.id, name: user.name, email: user.email, usn: user.usn, college_name: user.college_name }
    });
  } catch (err) {
    console.error(err);
...
const getMe = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, name, email, usn, college_name, created_at FROM users WHERE id = $1',
      [req.user.id]
    );
    if (result.rows.length === 0) {
